...
const TEST_USERS = {
    STANDARD: {
        ...
        tier: 'standard',
        limit: config.rateLimiter.limits.standard,
    },
    HIGH: {
        userId: `test-high-${Date.now()}`,
        tier: 'high',
        limit: config.rateLimiter.limits.high,
    },
};
...
describe('Rate Limiter E2E Tests', () => {
    ...
    describe('Rate Limiting Logic', () => {
        ...
        test('High tier should have a higher limit than standard tier', async () => {
            ...
            if (standardKeys.length > 0) await redis.del(...standardKeys);
            if (highKeys.length > 0) await redis.del(...highKeys);
        });
    });
    ...
    describe('Sliding Window Behavior', () => {
        test('Sliding window behavior - basic validation', async () => {
            ...
            expect(resetTime).toBeLessThanOrEqual(config.rateLimiter.windowSizeSeconds);
